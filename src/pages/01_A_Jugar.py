import cv2
import pandas as pd
import streamlit as st
from keras.datasets import mnist
from PIL import Image
from streamlit_drawable_canvas import st_canvas
import joblib


# Cargar los modelos entrenados
modelo_numeros = joblib.load("src/models/output/mejor_modelo_numeros.joblib")
modelo_operadores = joblib.load("src/models/output/mejor_modelo_operadores.joblib")

im = Image.open("favicon.ico")

if "number" not in st.session_state:
    st.session_state["number"] = 0

@st.cache_data
def get_mnist_data():
    return mnist.load_data()

def transform_image_to_mnist(image):
    st.write("Transform > Dimensiones de imagen de entrada")
    st.write(image.shape)
    if image.shape[2] == 4:
        st.write("Transform > Remover canal alpha")
        image = image[:, :, :3]
        st.write(image.shape)

    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    st.write("Transform > Conversion escala de grises")
    st.write(gray_image.shape)

    resized_image = cv2.resize(gray_image, (28, 28), interpolation=cv2.INTER_AREA)
    equalized_image = cv2.equalizeHist(resized_image)
    st.write("Transform > Dimensiones imagen de salida")
    st.write(resized_image.shape)

    return resized_image, equalized_image

def reconocer_numero(imagen):
    imagen_mnist, _ = transform_image_to_mnist(imagen)
    imagen_mnist = imagen_mnist.reshape(1, 28, 28, 1).astype('float32') / 255
    numero = modelo_numeros.predict(imagen_mnist)
    return numero.argmax()

def reconocer_operador(imagen):
    imagen_mnist, _ = transform_image_to_mnist(imagen)
    imagen_mnist = imagen_mnist.reshape(1, 28, 28, 1).astype('float32') / 255
    operador = modelo_operadores.predict(imagen_mnist)
    return operador.argmax()

def mnist_dataset_viewer(x_train, y_train, x_test, y_test):
    st.header("Sección de mnist")
    option = st.sidebar.selectbox("De cuál dataset quieres ver la imagen?", ("train", "test"))

    if option == "train":
        st.session_state["number"] = st.sidebar.slider(
            "Índice de la imagen en entrenamiento", 0, x_train.shape[0], 0
        )
        st.image(x_train[st.session_state["number"]], channels="gray")
    else:
        st.session_state["number"] = st.sidebar.slider(
            "Índice de la imagen en prueba", 0, x_test.shape[0], 0
        )
        st.image(x_test[st.session_state["number"]], channels="gray")

    st.write("Shape of mnist image")
    st.write(x_train[st.session_state["number"]].shape)

def play_canvas():
    (x_train, y_train), (x_test, y_test) = get_mnist_data()
    stroke_width = st.sidebar.slider("Stroke width: ", 1, 25, 3)
    stroke_color = st.sidebar.color_picker("Stroke color hex: ")
    bg_color = st.sidebar.color_picker("Background color hex: ", "#eee")
    realtime_update = st.sidebar.checkbox("Update in realtime", True)

    with st.container():
        (
            number_one, _, operator_one, number_two, _, operator_two, number_three
        ) = st.columns([3, 1, 2, 3, 1, 2, 3])

        with number_one:
            c1, c2 = st.columns(2)
            with c1:
                st.empty()
            with c2:
                exponent_1 = st_canvas(
                    fill_color="rgba(255, 165, 0, 0.3)",
                    stroke_width=stroke_width,
                    stroke_color=stroke_color,
                    background_color=bg_color,
                    update_streamlit=realtime_update,
                    height=50,
                    width=50,
                    drawing_mode="freedraw",
                    point_display_radius=0,
                    key="exponent_1",
                )

            number_1 = st_canvas(
                fill_color="rgba(255, 165, 0, 0.3)",
                stroke_width=stroke_width,
                stroke_color=stroke_color,
                background_color=bg_color,
                update_streamlit=realtime_update,
                height=150,
                width=150,
                drawing_mode="freedraw",
                point_display_radius=0,
                key="number_1",
            )

        with operator_one:
            with st.container():
                st.markdown("#")
                st.markdown("#")
                operator_1 = st_canvas(
                    fill_color="rgba(255, 165, 0, 0.3)",
                    stroke_width=stroke_width,
                    stroke_color=stroke_color,
                    background_color=bg_color,
                    update_streamlit=realtime_update,
                    height=100,
                    width=100,
                    drawing_mode="freedraw",
                    point_display_radius=0,
                    key="operator_1",
                )
        with number_two:
            c1, c2 = st.columns(2)
            with c1:
                st.empty()
            with c2:
                exponent_2 = st_canvas(
                    fill_color="rgba(255, 165, 0, 0.3)",
                    stroke_width=stroke_width,
                    stroke_color=stroke_color,
                    background_color=bg_color,
                    update_streamlit=realtime_update,
                    height=50,
                    width=50,
                    drawing_mode="freedraw",
                    point_display_radius=0,
                    key="exponent_2",
                )
            number_2 = st_canvas(
                fill_color="rgba(255, 165, 0, 0.3)",
                stroke_width=stroke_width,
                stroke_color=stroke_color,
                background_color=bg_color,
                update_streamlit=realtime_update,
                height=150,
                width=150,
                drawing_mode="freedraw",
                point_display_radius=0,
                key="number_2",
            )

        with operator_two:
            st.markdown("#")
            st.markdown("#")
            operator_2 = st_canvas(
                fill_color="rgba(255, 165, 0, 0.3)",
                stroke_width=stroke_width,
                stroke_color=stroke_color,
                background_color=bg_color,
                update_streamlit=realtime_update,
                height=100,
                width=100,
                drawing_mode="freedraw",
                point_display_radius=0,
                key="operator_2",
            )

        with number_three:
            c1, c2 = st.columns(2)
            with c1:
                st.empty()
            with c2:
                exponent_3 = st_canvas(
                    fill_color="rgba(255, 165, 0, 0.3)",
                    stroke_width=stroke_width,
                    stroke_color=stroke_color,
                    background_color=bg_color,
                    update_streamlit=realtime_update,
                    height=50,
                    width=50,
                    drawing_mode="freedraw",
                    point_display_radius=0,
                    key="exponent_3",
                )

            number_3 = st_canvas(
                fill_color="rgba(255, 165, 0, 0.3)",
                stroke_width=stroke_width,
                stroke_color=stroke_color,
                background_color=bg_color,
                update_streamlit=realtime_update,
                height=150,
                width=150,
                drawing_mode="freedraw",
                point_display_radius=0,
                key="number_3",
            )

    number, exponent, operator, mnist = st.tabs(
        ["Número", "Exponente", "Operador", "Mnist"]
    )

    with number:
        st.header("Sección de Número")
        if number_1.image_data is not None:
            st.write("Imagen dibujada:")
            st.image(number_1.image_data)

            numero_reconocido = reconocer_numero(number_1.image_data)
            st.write(f"Número reconocido: {numero_reconocido}")

    with exponent:
        st.header("Sección de Exponente")
        if exponent_1.image_data is not None:
            st.write("Imagen dibujada:")
            st.image(exponent_1.image_data)

            exponente_reconocido = reconocer_numero(exponent_1.image_data)
            st.write(f"Exponente reconocido: {exponente_reconocido}")

    with operator:
        st.header("Sección de Operador")
        if operator_1.image_data is not None:
            st.write("Operador dibujado:")
            st.image(operator_1.image_data)

            operador_reconocido = reconocer_operador(operator_1.image_data)
            st.write(f"Operador reconocido: {operador_reconocido}")

    with mnist:
        mnist_dataset_viewer(x_train, y_train, x_test, y_test)

def main():
    play_canvas()

if __name__ == "__main__":
    main()
